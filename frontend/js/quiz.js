...
async function initQuiz() {
  ...
  // Get the currently selected subject directly
  const subjectSelect =
  ...
  const modeSel =
    document.getElementById("quizModeSelect")?.value ||
    "Quick Quiz";

  const numQs =
    parseInt(
      document.getElementById("quizNumInput")?.value || 5
    );

  const diff =
    document.getElementById("quizDiffSelect")?.value ||
    "Adaptive";
  ...
  if (
    ...
    alert(
      "No quiz questions available for this subject. Upload study material first."
    );
  ...
  // IMPORTANT: save the actual quiz ID
  ...
  questionCard.innerHTML = `
    ...
    ">
      ...
      <span class="badge badge-medium">
        ${q.unit || "General"} • ${q.topic || "General"}
      </span>
    ...
    ">
      ...
      <button
        id="quizSubmitNextBtn"
        class="btn btn-primary"
        onclick="submitOrNextQuestion()"
      >
...
async function finishQuiz() {
  ...
  if (!currentQuizId) {
    ...
    alert(
      "Quiz session was not created correctly."
    );
